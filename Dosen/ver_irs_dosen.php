...
<body>
  ...
  if(isset($_SESSION['alert'])){
    echo '<script>alert("File IRS belum diupload!");</script>';
  }
        ...
        <div class="recent-sales box" style="padding-bottom: 100px; ">
          ...
          <div>
            <table class="customers">
              ...
              <?php 
              $query = "SELECT nama_mhs, irs.NIM AS nim, angkatan, smt, jml_sks, irs.status AS status, validasi FROM mahasiswa, irs WHERE mahasiswa.NIM = irs.NIM AND NIP_doswal = '$NIP' ORDER BY validasi, smt DESC";
              $result = $koneksi->query( $query );
              if (!$result){
                ...
                while ($row = $result->fetch_object()){ ?>
                  <tr>
                    ...
                    <td>
                    <!-- tombol validasi -->
                    <a href="update_data.php?val=<?php echo 'check_irs'; ?>&nip=<?php echo $NIP; ?>&nim=<?php echo $row->nim; ?>&smt=<?php echo $row->smt;?>" style="color: #081D45; border:none;"><i class='bx bxs-check-circle bx-sm'></i></a>
                    <a href="update_data.php?val=<?php echo 'uncheck_irs'; ?>&nip=<?php echo $NIP; ?>&nim=<?php echo $row->nim; ?>&smt=<?php echo $row->smt;?>" style="color: #081D45; border:none;"><i class='bx bx-window-close bx-sm'></i></a>
                    <!-- tombol edit -->
                    <a href="ver_irs_edit.php?nip=<?php echo $NIP; ?>&nim=<?php echo $row->nim; ?>&smt=<?php echo $row->smt;?>"  style="color: #081D45;"><i class='bx bxs-pencil bx-sm'></i></a>
                    <!-- tombol view -->
                    <a href="view_doc.php?val=<?php echo 'irs' ?>&nip=<?php echo $NIP ?>&nim=<?php echo $row->nim;?>&smt=<?php echo $row->smt;?>" style="color: #081D45;"><i class='bx bxs-file-find bx-sm' ></i></a>
                    </td>
                  </tr>
              <?php      
                }
